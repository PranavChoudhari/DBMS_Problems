...
select stcode1,stcode1 from track where distance < 20;

select id from trainhalts where stcode = 'TNA';

select name from train where id in (select id from trainhalts inner join station on trainhalts.stcode = station.stcode where station.name = 'MUMBAI');
